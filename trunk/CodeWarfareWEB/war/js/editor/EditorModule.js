...
//alert(src);
		if(editor.target){
			var module = ModuleLoader.getModule(editor.target);
			if(module){
				...
				src = src.replace(new RegExp('this.','g'),'MainApp.getModule(\''+module.id+'\').')
				for(var i=0;i<json.getCount();i++){
					src = src.replace(new RegExp(json.getAt(i).get('mapping')+'[\(]','g'),'MainApp.getModule(\''+module.id+'\').'+json.getAt(i).get('method')+'(')
				}
				...
				if(beginMethod){
					beginMethod = 'MainApp.getModule(\''+module.id+'\').'+beginMethod+'()';
