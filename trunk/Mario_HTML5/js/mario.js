...
var Mario = {
    ...
    SpriteCuts:{
        ...
        GetBackgroundSheet:function () {
            var a = [];
            var c = 0;
            var e = Engine.Resources.Images.background.width / 32;
            var d = Engine.Resources.Images.background.height / 32;
            for (var b = 0; b < e; b++) {
                a[b] = [];
                for (c = 0; c < d; c++) {
        ...
        GetLevelSheet:function () {
            var a = [];
            var c = 0;
            var e = Engine.Resources.Images.map.width / 16;
            var d = Engine.Resources.Images.map.height / 16;
            for (var b = 0; b < e; b++) {
                a[b] = [];
                for (c = 0; c < d; c++) {
    ...
    Level:function (width, height) {
        ...
        for (var i = 0; i < this.Width; i++) {
            this.Map[i] = [];
            this.Data[i] = [];
            this.SpriteTemplates[i] = [];
            for (var j = 0; j < this.Height; j++) {
                this.Map[i][j] = 0;
                this.Data[i][j] = 0;
                this.SpriteTemplates[i][j] = null
            }
        }
    }
};
...
Mario.Level.prototype = {
    ...
    SetBlock:function (x, y, block) {
        x < 0 || y < 0 || x >= this.Width || y >= this.Height || (this.Map[x][y] = block)
    },

    /**
     * 设置方块数据
     */
    SetBlockData:function (x, y, block) {
        x < 0 || y < 0 || x >= this.Width || y >= this.Height || (this.Data[x][y] = block)
    },
...
Mario.BackgroundGenerator.prototype = {
    ...
    GenerateOverground:function (a) {
        var b = this.Distant ? 4 : 6;
        var c = this.Distant ? 2 : 1;
        var e = Math.floor(Math.random() * b) + c;
...
Mario.BackgroundRenderer.prototype.Draw = function (a, b) {
    for (var c = b.X / this.Distance, d = 0, f = 0, g = (c + this.Width) / 32 | 0, e = c / 32 | 0; e <= g; e++) {
        for (d = 0; d < this.TilesY; d++) {
            f = this.Level.GetBlock(e, d) & 255;
            f = this.Background[f % 8][f / 8 | 0];
            a.drawImage(Engine.Resources.Images.background, f.X, f.Y, f.Width, f.Height, (e << 5) - c | 0, d << 5 | 0, f.Width, f.Height);
        }
    }
};
...
Mario.ImprovedNoise.prototype = {
    Shuffle:function () {
        ...
        for (b = 0; b < 256; b++) {
            c = (Math.random() * 255 | 0) + b, e = a[b], a[b] = a[c], a[c] = e, this.P[b + 256] = this.P[b] = a[b]
        }
    },
    ...
    Noise:function (a, b, c) {
        ...
        return this.Lerp(
            ...
            this.Lerp(
                ...
                this.Lerp(
                    g,
                    this.Grad(this.P[k], a, b, c),
                    this.Grad(this.P[e], a - 1, b, c)
                ),
                this.Lerp(
                    g,
                    this.Grad(this.P[j], a, b - 1, c),
                    this.Grad(this.P[f], a - 1, b - 1, c)
                )
            ),
            this.Lerp(
                h,
                this.Lerp(
                    g,
                    this.Grad(this.P[k + 1], a, b, c - 1),
                    this.Grad(this.P[e + 1], a - 1, b, c - 1)
                ),
                this.Lerp(
                    g,
                    this.Grad(this.P[j + 1], a, b - 1, c - 1),
...
Mario.Character = function () {
    ...
    this.Coins = 0;         //金币
    this.Lives = 3;         //生命
    this.LevelString = "none";  //关卡名
    this.AirInertia = this.GroundInertia = 0.89;           //空中惯性、地面惯性
    this.RunTime = 0;
...
Mario.Character.prototype.Blink = function (a) {
    ...
    if (this.Large) {
        this.Image = this.Fire ? Engine.Resources.Images.fireMario : Engine.Resources.Images.mario;
        this.XPicO = 16;
        this.YPicO = 31;
...
Mario.Character.prototype.Move = function () {
    if (this.WinTime > 0) {
        ...
        if (this.OnGround) {
            this.Ducking = Engine.KeyboardInput.IsKeyDown(Engine.Keys.Down) && this.Large ? !0 : !1;
        }
        if (this.Xa > 2) {
            this.Facing = 1;
        }
        ...
        if (this.Sliding) {
            this.World.AddSprite(new Mario.Sparkle(this.World, (this.X + Math.random() * 4 - 2 | 0) + this.Facing * 8, (this.Y + Math.random() * 4 | 0) - 24, Math.random() * 2 - 1, Math.random(), 0, 1, 5));
            this.Ya *= 0.5;
        }
        ...
        if (this.Carried !== null && (this.Carried.X *= this.X + this.Facing * 8, this.Carried.Y *= this.Y - 2, !Engine.KeyboardInput.IsKeyDown(Engine.Keys.A))) {
...
Mario.Character.prototype.CalcPic = function () {
    ...
    if (this.Large) {
        ...
        a === 3 && (a = 1);
        this.Carried === null && Math.abs(this.Xa) > 10 && (a += 3), this.Carried !== null && (a += 10), this.OnGround || (a = this.Carried !== null ? 12 : Math.abs(this.Xa) > 10 ? 7 : 6);
    } else {
        a = (this.RunTime / 20 | 0) % 2;
        this.Carried === null && Math.abs(this.Xa) > 10 && (a += 2), this.Carried !== null && (a += 8), this.OnGround || (a = this.Carried !== null ? 9 : Math.abs(this.Xa) > 10 ? 5 : 4);
    }
...
Mario.Character.prototype.SubMove = function (a, b) {
    ...
    if (b > 0) {
        if (this.IsBlocking(this.X + a - this.Width, this.Y + b, a, 0) ||
...
Mario.LevelRenderer.prototype.DrawStatic = function (a, b) {
    for (var c = 0, e = 0, d = 0, d = null, f = (b.X + this.Width) / 16 | 0, c = b.X / 16 | 0; c < f + 1; c++) {
        for (e = 0; e < this.TilesY; e++) {
            ...
            if ((Mario.Tile.Behaviors[d] & Mario.Tile.Animated) === 0) {
                d = this.Background[d % 16][d / 16 | 0];
                a.drawImage(Engine.Resources.Images.map, d.X, d.Y, d.Width, d.Height, (c << 4) - b.X | 0, e << 4 | 0, d.Width, d.Height)
            }
        }
    }
};
Mario.LevelRenderer.prototype.DrawDynamic = function (a, b) {
    for (var e = 0, d = 0, f = 0, g = 0, c = b.X / 16 | 0; c <= (b.X + this.Width) / 16 | 0; c++) {
        for (e = b.Y / 16 | 0; e <= (b.Y + this.Height) / 16 | 0; e++) {
            d = this.Level.GetBlock(c, e);
            if ((Mario.Tile.Behaviors[d & 255] & Mario.Tile.Animated) > 0) {
                f = (this.Bounce / 3 | 0) % 4;
...
Mario.LevelRenderer.prototype.DrawExit0 = function (a, b, c) {
    for (var e = 0, e = 0, d = null, e = this.Level.ExitY - 8; e < this.Level.ExitY; e++) {
        ...
        a.drawImage(Engine.Resources.Images.map, d.X, d.Y, d.Width, d.Height, (this.Level.ExitX << 4) - b.X - 16, (e << 4) - b.Y, d.Width, d.Height);
    }
    if (c) {
...
Mario.LevelRenderer.prototype.DrawExit1 = function (a, b) {
    for (var c = 0, e = null, c = this.Level.ExitY - 8; c < this.Level.ExitY; c++) {
        ...
        a.drawImage(Engine.Resources.Images.map, e.X, e.Y, e.Width, e.Height, (this.Level.ExitX << 4) - b.X + 16, (c << 4) - b.Y, e.Width, e.Height);
    }
};
...
Mario.LevelGenerator.prototype = {
    ...
    BuildHillStraight:function (a, b, c) {
        ...
        var d = this.Height - 1 - Math.random() * 4 | 0;
        var f = 0;
        var g = 0;
        var h = d;
        var i = !0;
        ...
        for (this.AddEnemyLine(a, b + 1, b + e - 1, d - 1); i;) {
            if (h = h - 2 - Math.random() * 3 | 0, h <= 0) {
                i = !1;
            } else if (j = (Math.random() * 5 | 0) + 3, k = (Math.random() * (e - j - 2) | 0) + b + 1, l[k - b] || l[k - b + j] || l[k - b - 1] || l[k - b + j + 1]) {
                i = !1;
            } else {
                l[k - b] = !0;
                l[k - b + j] = !0;
            ...
            for (h = 0; h < e; h++) {
                ...
                if (f[0][0] === f[1][0] && f[0][1] === f[1][1]) {
                    if (f[0][0] === f[0][1]) {
                        ...
                        f[0][0] ? a.SetBlock(g, h, 161 + d) : a.SetBlock(g, h, 129 + d)
                    }
                } else if (f[0][0] === f[0][1] && f[1][0] === f[1][1]) {
                    f[0][0] ? a.SetBlock(g, h, 146 + d) : a.SetBlock(g, h, 144 + d)
                } else if (f[0][0] === f[1][1] && f[0][1] === f[1][0]) {
                    a.SetBlock(g, h, 145 + d)
                } else if (f[0][0] === f[1][0]) {
                    if (f[0][0]) {
                        f[0][1] ? a.SetBlock(g, h, 163 + d) : a.SetBlock(g, h, 179 + d)
                    } else {
                        f[0][1] ? a.SetBlock(g, h, 130 + d) : a.SetBlock(g, h, 128 + d)
                    }
                } else if (f[0][1] === f[1][1]) {
                    if (f[0][1]) {
                        f[0][0] ? a.SetBlock(g, h, 147 + d) : a.SetBlock(g, h, 131 + d)
                    } else {
                        f[0][0] ? a.SetBlock(g, h, 162 + d) : a.SetBlock(g, h, 160 + d)
                    }
                } else {
...
Mario.SpriteTemplate.prototype = {
    Spawn:function (a, b, c, e) {
        if (!this.IsDead) {
            this.Sprite = this.Type === Mario.Enemy.Flower ? new Mario.FlowerEnemy(a, b * 16 + 15, c * 16 + 24) : new Mario.Enemy(a, b * 16 + 8, c * 16 + 15, e, this.Type, this.Winged), this.Sprite.SpriteTemplate = this, a.AddSprite(this.Sprite)
        }
    }
};
...
Mario.Enemy.prototype.CollideCheck = function () {
    if (this.DeadTime === 0) {
        ...
        var b = Mario.MarioCharacter.Y - this.Y;
...
Mario.BulletBill.prototype.CollideCheck = function () {
    if (!this.Dead) {
        ...
        if (a > -16 && a < 16 && b > -this.Height && b < this.World.Mario.Height) {
            if (Mario.MarioCharacter.Y > 0 && b <= 0 && (!Mario.MarioCharacter.OnGround || !Mario.MarioCharacter.WasOnGround)) {
...
Mario.FlowerEnemy.prototype.Move = function () {
    ...
    if (this.DeadTime > 0) {
        ...
        if (this.Y >= this.YStart) {
            this.YStart = this.Y;
            a = Math.abs(Mario.MarioCharacter.X - this.X) | 0, this.JumpTime++;
    ...
    if (!this.Carried && !(this.Dead || this.DeadTime > 0)) {
        ...
        if (a > -16 && a < 16 && b > -this.Height && b < Mario.MarioCharacter.Height) {
            if (Mario.MarioCharacter.Ya > 0 && b <= 0 && (!Mario.MarioCharacter.OnGround || !Mario.MarioCharacter.WasOnGround)) {
                ...
                this.Facing = this.Facing !== 0 ? this.Xa = 0 : Mario.MarioCharacter.Facing
            } else {
                this.Facing !== 0 ? Mario.MarioCharacter.GetHurt() : (Mario.MarioCharacter.Kick(this), this.Facing = Mario.MarioCharacter.Facing)
            }
        }
    }
};
...
Mario.TitleState.prototype.Enter = function () {
    ...
    var b = new Mario.BackgroundRenderer(a.CreateLevel(), 320, 240, 2);
...
Mario.WinState.prototype.Update = function (a) {
    ...
    if (this.waitTime > 0) {
        this.waitTime -= a;
    } else if (Engine.KeyboardInput.IsKeyDown(Engine.Keys.S)) {
        this.wasKeyDown = !0
    }
};
...
Mario.MapState.prototype.GenerateLevel = function () {
    ...
    var c = 0;
    var e = 0;
    var d = new Mario.ImprovedNoise(Math.random() * 9223372036854775E3 | 0);
    var f = new Mario.ImprovedNoise(Math.random() * 9223372036854775E3 | 0);
    var g = new Mario.ImprovedNoise(Math.random() * 9223372036854775E3 | 0);
...
Mario.MapState.prototype.Connect = function (a, b, c, e) {
    for (var d = 1E4, f = 0, g = 0, h = 0, i = 0, j = 0, k = 0, h = j = 0; h < c; h++) {
        for (i = 0; i < e; i++) {
            if (this.Level[h][i] === Mario.MapTile.Level && this.Data[h][i] === -2) {
                j = Math.abs(a - h) | 0, k = Math.abs(b - i) | 0;
                j = j * j + k * k;
                j < d && (f = h, g = i, d = j);
            }
        }
...
Mario.MapState.prototype.FindCaps = function (a, b) {
    for (var c = 0, e = 0, d = -1, f = -1, g = 0, h = 0, i = 0, c = 0; c < a; c++) {
        for (e = 0; e < b; e++)if (this.Level[c][e] === Mario.MapTile.Level) {
            g = 0;
            for (h = c - 1; h <= c + 1; h++) {
                for (i = e - 1; i <= e + 1; i++) {
    ...
    if (!(this.Level[a][b] !== Mario.MapTile.Road && this.Level[a][b] !== Mario.MapTile.Level)) {
        ...
        if (this.Level[a][b] === Mario.MapTile.Level) {
            if (this.Data[a][b] > 0) {
                ...
                this.Farthest = e, this.XFarthestCap = a, this.YFarthestCap = b;
            }
        }
...
Mario.MapState.prototype.RenderStatic = function () {
    for (var a = 0, b = 0, c = 0, e = 0, d = 0, f = 0, g = c = 0, h = 0, i = Engine.Resources.Images.worldMap, a = g = 0; a < 20; a++) {
        for (b = 0; b < 15; b++) {
            this.MapContext.drawImage(i, (this.WorldNumber / 4 | 0) * 16, 0, 16, 16, a * 16, b * 16, 16, 16);
            if (this.Level[a][b] === Mario.MapTile.Level) {
                g = this.Data[a][b];
                if (g === 0) {
                    ...
                    this.MapContext.drawImage(i, 32, 112, 16, 16, a * 16, (b - 1) * 16, 16, 16);
                    this.MapContext.drawImage(i, 32, 128, 16, 16, a * 16, b * 16, 16, 16)
                } else {
...
Mario.MapState.prototype.Update = function (a) {
    ...
    if (this.WorldNumber !== 8) {
        ...
        if (this.MoveTime > 0) {
            ...
            if (this.CanEnterLevel && Engine.KeyboardInput.IsKeyDown(Engine.Keys.S) && this.Level[b][c] === Mario.MapTile.Level && this.Data[b][c] !== -11 && this.Level[b][c] === Mario.MapTile.Level && this.Data[b][c] !== 0 && this.Data[b][c] > -10) {
...
Mario.LevelState.prototype.Update = function (a) {
    ...
    for (b = this.FireballsOnScreen; b < this.Sprites.Objects.length; b++) {
...
Mario.LevelState.prototype.Bump = function (a, b, c) {
    ...
    if ((Mario.Tile.Behaviors[e & 255] & Mario.Tile.Bumpable) > 0) {
        ...
        if ((Mario.Tile.Behaviors[e & 255] & Mario.Tile.Special) > 0) {
            ...
            if (Mario.MarioCharacter.Large) {
                this.AddSprite(new Mario.FireFlower(this, a * 16 + 8, b * 16 + 8))
            } else {
                this.AddSprite(new Mario.Mushroom(this, a * 16 + 8, b * 16 + 8))
            }
        } else {
...
Mario.LevelState.prototype.BumpInto = function (a, b) {
    ...
    if ((Mario.Tile.Behaviors[c & 255] & Mario.Tile.PickUpable) > 0) {
        Mario.MarioCharacter.GetCoin();
        Engine.Resources.PlaySound("coin");
        this.Level.SetBlock(a, b, 0);
